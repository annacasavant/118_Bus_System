...
load_DA_TS = DataFrame()
for file in readdir("Scripts-and-Data/TimeSeries/DA/Load/", join=true)
    data = CSV.read(file, DataFrame)[:, 2]
    load_DA_TS[:, file[end-7:end-6]] = data
end

# building and adding loads to system

for row in eachrow(load_params)
    max = maximum(load_DA_TS[:, row[REGION]])
    load = PowerLoad(;
        name = "load$(row[LOAD_BUS][4:6])",
        available = true,
        bus = get_bus(sys_DA, row[LOAD_BUS]),
...
for reg in regions
    max = maximum(load_DA_TS[:, reg])
    local load_array = TimeArray(timestamps, (load_DA_TS[:, reg] ./ max))
    local load_TS = SingleTimeSeries(;
           name = "max_active_power",
           data = load_array,
           scaling_factor_multiplier = get_max_active_power, #assumption?
       );  
    region = get_component(Area, sys_DA, reg)
