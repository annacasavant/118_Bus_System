...
hydrogens = collect(get_components(HydroDispatch, sys_RT))
for i in 1:43 
...
for i in 1:192
    ...
    min_active_power = thermal_gens[i, "Min Stable Level (MW)"]/100
        thermal = ThermalStandard(;
            ...
            available = true,
            status = true,
            bus = get_bus(sys_RT, bus_thermal),
            active_power = 0.0,
            reactive_power = 0.0,
            rating = ratings[i],
